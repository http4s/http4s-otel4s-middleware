...
object OTHttpTags {
  object Common {
    ...
    def target(url: Uri): Attribute[String] =
      Attribute("http.target", url.copy(scheme = None, authority = None).renderString)
    def host(host: org.http4s.headers.Host): Attribute[String] =
      Attribute("http.host", org.http4s.headers.Host.headerInstance.value(host))
    ...
    def flavor(httpVersion: HttpVersion): Attribute[String] =
      Attribute("http.flavor", httpVersion.major.toString() ++ "." ++ httpVersion.minor.toString())
    def userAgent(userAgent: `User-Agent`): Attribute[String] =
      Attribute("http.user_agent", `User-Agent`.headerInstance.value(userAgent))
    def requestContentLength(cl: Long): Attribute[Long] =
      Attribute("http.request_content_length", cl)
    def responseContentLength(cl: Long): Attribute[Long] =
      Attribute("http.response_content_length", cl)
    def retryCount(i: Int): Attribute[Long] = Attribute("http.retry_count", i.toLong)
    def peerIp(ip: IpAddress): Attribute[String] =
      Attribute("net.peer.ip", ip.toString()) // TODO: Check that this is the right way
    def peerPort(port: Port): Attribute[Long] = Attribute("net.peer.port", port.value.toLong)
  }

  object Client {
    def peerName(uri: Uri): Option[Attribute[String]] =
      uri.host.map(h => Attribute("net.peer.name", h.value))
  }
  ...
  object Headers {
    ...
    private def generic(
      ...
      headers.headers
        ...
        .toList
        .map { case (name, list) =>
          val key = "http." ++ messageType ++ ".header.string." ++ name.toString.toLowerCase
            .replace("-", "_")
    ...
    // We add a string as a prefix, because the otel standard is an array so
    ...
    lazy val defaultHeadersIncluded: Set[CIString] = Set(
      "WWW-Authenticate",
      "Proxy-Authenticate",
      "Age",
      "Cache-Control",
